...
import edu.mit.broad.genome.utils.ExecLogHandler;
...
public class CytoscapeLaunch {
    ...
    public void launch() {
        ...
        String cytoscapeRootLocation = CytoscapeLocationSingleton.getInstance().getCytoscapeRootLocation();
        if (cytoscapeRootLocation != null && this.java != null) {
            try {
                String cytoscape;
                File workingDir = new File(cytoscapeRootLocation);
                /*
                 * launch cytoscape with -R option to open up rest capabilities
                 */
                if (SystemUtils.IS_OS_WINDOWS) {
                    cytoscape = "cmd /c start /B cytoscape.bat -R 1234";
                } else {
                    cytoscape = cytoscapeRootLocation + this.file_separator + "cytoscape.sh -R 1234";
                }
                ...
                if (CytoscapeLocationSingleton.getInstance().getMain_version() == 2) {
                    Application.getWindowManager()
                            .showConfirm("Cytoscape 2 is not supported by GSEA.  Please install latest version of Cytoscape");
                }
                ...
                klog.info("about to run command:" + launch_command);
                CommandLine cli = CommandLine.parse(launch_command);
                Executor exec = new DefaultExecutor();
                exec.setWorkingDirectory(workingDir);
                DefaultExecuteResultHandler resultHandler = new DefaultExecuteResultHandler();
                exec.setStreamHandler(new PumpStreamHandler(new ExecLogHandler(klog)));
                exec.execute(cli, resultHandler);
            ...
            klog.info("command finished:" + launch_command);

        } else {
            klog.info("cytoscape root or java is null:" + this.java + ","
                    + cytoscapeRootLocation);
        }
     ...
     * 
     * Given the name of the jar to get returns - the path to the jar on the current system
     */
    private String installPlugin(String name) {
        ...
        String cyto_home = home_dir + this.file_separator + "CytoscapeConfiguration" + this.file_separator
                + CytoscapeLocationSingleton.getInstance().getMain_version() + this.file_separator + "apps" + this.file_separator
                + "installed";
        ...
        } else klog.info("Cytoscape Configuration not set.  Nowhere to install the app to");
        return null;
    }

    /*
     * Check to see if the given cytoscape app jar is installed on the current system Return - version of the app installed on the system.
     * If it isn't installed return -1.
     */
